...
namespace NAQRPD.Common.Evaluation
{
    ...
    public class AQCIHelper
    {
        ...
        /// IAQCIReport单项质量指数属性
        /// </summary>
        private static Dictionary<string, PropertyInfo> IAQCIPropertiesDic;
        /// <summary>
        /// 污染物监测项浓度二级标准字典
        /// </summary>
        ...
        private static Dictionary<string, decimal> GetIAQCIDic(IAQCICalculate data)
        {
            ...
            foreach (PropertyInfo property in IAirDataProperties)
            {
                ...
                if (value != null)
                {
                    ...
                    if (decimalValue >= 0) IAQCIDic.Add(property.Name, Math.Round(decimalValue / limitDic[property.Name], 2));
                }
            }
            return IAQCIDic;
        }
